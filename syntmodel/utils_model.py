...
class Prms:
    ...
    def __init__(self,
        ...
        if param_dict:
            self.update_from_dict(param_dict)
        else:
            self.pa = pa
            self.qa = qa
            self.ra = ra
    ...
    def update_limits_from_params(self, params_list):
        param_keys = ['pa', 'qa', 'ra', 'sa', 'ta', 'pb', 'qb', 'rb', 'sb', 'tb',
                      'pd', 'qd', 'rd', 'sd', 'td', 'pm', 'qm', 'rm', 'sm', 'tm',
                      'mu', 'a', 'std', 'alpha', 'c', 'wn']

        for key in param_keys:
            values = [params[key] for params in params_list]
            setattr(self, f"{key}1", min(values))
            setattr(self, f"{key}2", max(values))
    def randomize(self):
